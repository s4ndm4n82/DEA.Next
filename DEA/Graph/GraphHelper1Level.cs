...
namespace DEAHelper1Leve
{
    internal class GraphHelper1LevelClass
    {
        public static async Task GetEmailsAttacments1Level([NotNull] GraphServiceClient graphClient, string _Email)
        {
                ...
                foreach (var FirstSubFolderID in FirstSubFolderIDs.Where(ids => !string.IsNullOrWhiteSpace(ids.Id)))
                {
                    ...
                    foreach (var Message in GetMessageAttachments)
                    {
                        ...
                        if (Count > 0 && System.IO.Directory.Exists(PathFullDownloadFolder) && System.IO.Directory.EnumerateFiles(PathFullDownloadFolder, "*", SearchOption.AllDirectories).Any())
                        {
                            ...
                            {
                                WriteLogClass.WriteToLog(3, "File/s moved successfully ....", string.Empty);
                                MoveToExport = true;
                            }
                            else
                            {
                                WriteLogClass.WriteToLog(3, "File/s was/were not moved successfully ....", string.Empty);
                            }
                        }
                        ...
                        if (MoveToExport)
                        {
                            ...
                            {
                                WriteLogClass.WriteToLog(1, $"Exception at attachment download area 1level: {ex.Message}", string.Empty);
                            }
                        }
            ...
            {
                WriteLogClass.WriteToLog(1, $"Exception at end of main foreach 1level: {ex.Message}", string.Empty);
            }
        }
    }
}