...
namespace GraphHelper
{
    public class GraphHelperClass
    {
        ...
        /// <returns></returns>
        public static async Task<int> InitializeGetAttachment(IUserConfigRepository configRepository,Guid customerId)
        {
            var result = 0;
            var clientDetails = await configRepository.GetEmailDetailsById(customerId);

            GetInboxFolderNames getInboxFolderNames = new(clientDetails.EmailInboxPath);
            ...
            if (string.IsNullOrEmpty(mainInbox)) return result;
            {
                try
                {
                    ...
                    if (_graphClient != null)
                        result = await GraphGetAttachmentsClass.StartAttachmentDownload(configRepository,
                            _graphClient,
                            clientDetails.Email!,
                            mainInbox,
                            subInbox1,
